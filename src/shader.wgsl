...
fn fs_main(
    ...
    let L: vec3<f32> = normalize(
        frag_uniforms.light_position.xyz - v_position.xyz
    );

    let V: vec3<f32> = normalize(
        frag_uniforms.eye_position.xyz - v_position.xyz
    );
    ...
    let specular: f32 = light_uniforms.params[2]
        * pow(max(dot(N, H), 0.0)
        , light_uniforms.params[3]);

    let ambient: f32 = light_uniforms.params[0];

    let final_color = light_uniforms.color
        * (ambient + diffuse)
        + light_uniforms.specular_color
        * specular;
